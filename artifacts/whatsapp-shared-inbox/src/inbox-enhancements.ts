...
async function activeConversation() {
  ...
  const match = (list || []).find((item: any) =>
    phone(item?.contact?.phoneNumber || '') === contactPhone ||
...
function styles() {
  ...
  style.textContent = `
    .fsf-tools{display:flex;gap:5px;margin-top:4px;opacity:0;transition:opacity .15s}
    ...
    .fsf-compose-btn:hover,.fsf-reply-btn:hover{background:#f0f6f1}
    .fsf-compose-btn.recording{background:#fce8e8;border-color:#e9b5b5;color:#a23b3b}
    ...
    .fsf-reply-bar button{margin-left:auto;border:0;background:transparent;cursor:pointer;font-size:16px;color:#55766c}
    ...
    .fsf-media-loading{font-size:10px;opacity:.7;padding:6px 0}
    .fsf-media-error{font-size:10px;color:#a23b3b}
    .fsf-audio{width:min(320px,100%)}
    .fsf-image{display:block;max-width:280px;max-height:240px;border-radius:8px;object-fit:contain;cursor:pointer}
    .fsf-video{display:block;max-width:320px;max-height:240px;border-radius:8px}
...
function setReply(id: string, text: string) {
  ...
  if (!bar) {
    ...
    bar.className = 'fsf-reply-bar';
    area.prepend(bar);
  }

  bar.innerHTML = `<b>↩</b><span>${escapeHtml(reply.text)}</span><button aria-label="Cancelar resposta">×</button>`;
  bar.querySelector('button')?.addEventListener('click', clearReply);
}

function clearReply() {
  reply = null;
  document.querySelector('.fsf-reply-bar')?.remove();
}
...
function addReplyButton(line: Element, message?: EnhancedMessage) {
  ...
  const text = line.querySelector('.bubble')?.textContent?.trim() || message?.content || 'Mensagem';
...
async function renderMedia(bubble: HTMLElement, message: EnhancedMessage) {
  ...
  bubble.innerHTML = '<span class="fsf-media-loading">Carregando mídia…</span>';

  try {
    const response = await fetch(
      `${API}/api/messages/${encodeURIComponent(message.id)}/media`,
      { headers: authHeaders() },
    );
    ...
    const type = String(message.mediaType || blob.type || '');
    bubble.innerHTML = '';
    ...
    bubble.innerHTML = `<span class="fsf-media-error">Não foi possível carregar esta mídia.</span>`;
  }
}
...
async function enhanceMessages() {
  ...
  try {
    ...
    thread.querySelectorAll('.message-line').forEach((line) => {
      ...
      if (
        message.mediaUrl &&
        line.querySelector('.bubble') &&
        !line.querySelector('.fsf-audio,.fsf-image,.fsf-video,.fsf-doc,.fsf-media-loading,.fsf-media-error')
      ) {
        void renderMedia(line.querySelector('.bubble') as HTMLElement, message);
      }
...
function showFile() {
  ...
  box.innerHTML = `<span>${file.type.startsWith('audio/') ? '🎙' : '📎'}</span><span>${escapeHtml(file.name)}</span><button aria-label="Remover anexo">×</button>`;
...
function appendSent(message: EnhancedMessage) {
  ...
  meta.textContent = new Date(message.createdAt || Date.now()).toLocaleTimeString('pt-BR', {
    hour: '2-digit',
    minute: '2-digit',
  });
...
function clearComposerTextarea(textarea: HTMLTextAreaElement) {
  textarea.value = '';
...
async function send() {
  ...
  try {
    ...
    const message = await api(
      `/api/conversations/${encodeURIComponent(id)}/messages/advanced`,
      {
        method: 'POST',
...
function composer() {
  ...
  root.insertBefore(tools, textarea);
}
...
export function installInboxEnhancements() {
  ...
  window.addEventListener('keydown', (event) => {
    ...
    if (
      event.key === 'Enter' &&
      !event.shiftKey &&
      target?.matches('.composer textarea')
    ) {
      event.preventDefault();
      event.stopPropagation();
