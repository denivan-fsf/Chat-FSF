...
async function mediaUrl(username: string, version: string, mediaId: string, token: string) {
...
function publicMessage(row: any) {
  return {
    ...
    sentByUser: row.sent_by_user_id ? {
      ...
      initials: String(row.sent_user_name || 'AT').split(/\s+/).filter(Boolean).slice(0, 2).map((x: string) => x[0]).join('').toUpperCase(),
...
// O frontend usa esta rota para obter as mensagens com suporte a resposta e mídia.
...
router.post('/conversations/:id/messages/advanced', requireUser, async (req: any, res: any) => {
  ...
  if (file) {
    ...
    mediaUrlValue = uploadData?.url || null;
    ...
    if (mimeType.startsWith('audio/')) {
      ...
      type = 'document';
      payload = { to, type: 'document', document: { id: mediaId, filename: name, caption: text || undefined } };
    }
  } else {
  ...
  const inserted = await pool.query(`
    ...
    returning *
  `, [req.params.id, text || (type === 'audio' ? 'Áudio' : 'Arquivo'), mediaUrlValue, mediaTypeValue, req.currentUser.id, replyId]);
  ...
  try {
    ...
    await pool.query('update public.conversations set last_message_preview=$2,last_message_at=now(),updated_at=now() where id=$1', [req.params.id, text || (type === 'audio' ? 'Áudio' : nameFromFile(file))]);
    ...
    const full = await pool.query(`
      ...
      from public.messages m left join public.workspace_users u on u.id=m.sent_by_user_id left join public.messages r on r.id=m.reply_to_message_id where m.id=$1
    `, [local.id]);
...
// A rota vem antes do webhook existente. Ela processa apenas mídias; textos/status seguem para o handler original.
router.post('/webhooks/whatsapp', async (req: any, res: any, next: any) => {
  try {
    ...
    for (const entry of entries) {
      for (const change of entry?.changes || []) {
        ...
        const mediaMessages = (Array.isArray(value?.messages) ? value.messages : []).filter((m: any) => ['audio','image','video','document'].includes(String(m?.type || '')));
        if (!mediaMessages.length) continue;

        let numberQ: any;
        if (phoneNumberId) numberQ = await pool.query('select id,uzapi_username from public.whatsapp_numbers where phone_number_id=$1 limit 1', [phoneNumberId]);
        if ((!numberQ || !numberQ.rows[0]) && displayPhone) numberQ = await pool.query('select id,uzapi_username from public.whatsapp_numbers where phone_number=$1 limit 1', [displayPhone]);
        ...
        for (const message of mediaMessages) {
          ...
          if (!mediaId) continue;
          const url = await mediaUrl(username, version, mediaId, token);
          const content = String(media?.caption || media?.filename || (type === 'audio' ? 'Áudio' : type === 'image' ? 'Imagem' : type === 'video' ? 'Vídeo' : 'Arquivo'));
          const ct = await pool.query('insert into public.contacts(name,phone_number) values($1,$2) on conflict(phone_number) do update set name=coalesce(nullif(excluded.name,\'Cliente\'),public.contacts.name),updated_at=now() returning id', [senderName, phone]);
          ...
          await pool.query('insert into public.messages(conversation_id,direction,content,media_url,media_type,status,provider_message_id,created_at) values($1,\'inbound\',$2,$3,$4,\'delivered\',$5,coalesce(to_timestamp($6),now()))', [cv.rows[0].id, content, url, String(media?.mime_type || type), providerId, message?.timestamp || null]);
